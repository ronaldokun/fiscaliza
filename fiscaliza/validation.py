...
# def get_url(teste=True):
#     """Mapeia a url do Redmine para os endereços tanto de homologação como produção"""
#     pub = True
#     url = URL_MAPPING.get((teste, pub))


def auth_user(
    username=None, password=None, api=None, teste=True, pub=True, verify=True
):
    ...
    fiscaliza = Redmine(
        url, key=api, username=username, password=password, requests={"verify": verify}
    )
    try:
        fiscaliza.auth()
        return fiscaliza
        ...
        if verify:
            return auth_user(username, password, api, teste, pub, verify=False)
    except ConnectionError:
        if pub:
            return auth_user(username, password, api, teste, False, verify)

    raise ConnectionError("Não foi possível conectar ao fiscaliza. Erro de Conexão")
...
def _validar_uf_municipio(dados, key):
    if municipio := dados.get(key):
        ...
        lista_municipios = []
        for m in municipio:
            if re.match(f'({"|".join(ESTADOS)})/(\w+[\s|\w]+)', m):
