...
__all__ = ['journal2table', 'value_text_string', 'check_update', 'view_string', 'valida_fiscaliza', 'issue_type',
           'hm2prod', 'validar_dicionario', 'auth_user', 'issue2users', 'insp2acao', 'utf2ascii', 'detalhar_inspecao',
...
def utf2ascii(string):
    string = re.sub('[/\?\.]', '', string)
    return unidecode(string.replace(' ', '_'))
...
def detalhar_inspecao(
    ...
    result["Atualizado"] = f"Atualizado por {user} em {datetime.strftime(date, '%d/%m/%Y')} às {date.time()}"
        ...
        with console.status(
            ...
            if status == "Relatada":
                try:
                    ...
                    console.print(
                        f":black_nib: Assine o Relatorio_de_Monitoramento: {status_atual.get('Relatorio_de_Monitoramento', '')} e chame a função novamente :exclamation:"
                    )
                    break
            else:
                atualiza_fiscaliza(inspecao, dados, fiscaliza, status)
